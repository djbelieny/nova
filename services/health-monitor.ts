  ...
  if (isMac) {
    for (const [label, desc] of Object.entries(SERVICES_MAC)) {
      try {
        ...
        if (code !== 0) {
          ...
          const isKeepAlive = label === "com.nova.core";
    ...
    for (const [svc, desc] of Object.entries(SERVICES_LINUX)) {
      try {
        ...
        if (stdout === "activating") {
          results.push({
            status: "ok",
            checkName: "checkServices",
            detail: `${desc} (${svc}) is starting up (activating)`,
          });
        } else if (stdout !== "active") {
          results.push({
            status: "critical",
            checkName: "checkServices",
            detail: `${desc} (${svc}) status: ${stdout} — systemd will restart automatically`,
          });
        } else {
          results.push({
            status: "ok",
...
async function autoFix(issue: HealthIssue): Promise<string> {
  ...
  if (issue.fixAction === "restart_service") {
    if (isMac) {
      ...
      const proc = Bun.spawn(
        ["systemctl", "restart", issue.fixTarget],
...
// ============================================================
// ZOMBIE DETECTION MONITOR
// Polls /health every 2 minutes and alerts admin after 3
// consecutive failures. Imported and started by relay.ts.
// ============================================================

import { notifyAdmin } from "../src/error-handler.ts";

const HEALTH_CHECK_INTERVAL = 2 * 60 * 1000; // 2 minutes
const FAILURE_THRESHOLD = 3;
const HEALTH_ENDPOINT = process.env.HEALTH_ENDPOINT || "http://localhost:3033/health";
...
export async function startHealthMonitor(): Promise<void> {
  setInterval(async () => {
    try {
      const response = await fetch(HEALTH_ENDPOINT, { signal: AbortSignal.timeout(10_000) });
      if (response.ok) {
        if (consecutiveFailures > 0) {
          consecutiveFailures = 0;
          await notifyAdmin("Nova health restored ✅");
        }
      } else {
        consecutiveFailures++;
        if (consecutiveFailures === FAILURE_THRESHOLD) {
          await notifyAdmin(
            `Nova health check failed ${consecutiveFailures} times. Status: ${response.status}. Check the server.`
          );
        }
      }
    } catch {
      ...
      if (consecutiveFailures === FAILURE_THRESHOLD) {
        await notifyAdmin(
          `Nova health check unreachable ${consecutiveFailures} times. Server may be down.`
        );
      }
    }
