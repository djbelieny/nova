...
-- Row Level Security
-- ============================================================
-- All board tables are accessed exclusively by trusted server-side
-- executive nodes using the service role key (which bypasses RLS).
-- Enabling RLS with no policies blocks all access via the public
-- anon key — never expose these tables to browser/anon clients.

ALTER TABLE exec_nodes          ENABLE ROW LEVEL SECURITY;
