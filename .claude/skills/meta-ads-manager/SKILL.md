...
**Connected pages:** Discover at runtime with `getPages` (or `bun run src/meta-ads.ts pages`) — lists the Facebook pages your access token is connected to.

**Ad Account:** Set `META_AD_ACCOUNT_ID` in `.env` (format `act_XXXXXXXXX`). Use `getAdAccount` to confirm currency and timezone.
