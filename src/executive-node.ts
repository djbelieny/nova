...
async function main() {
  ...
  initExecHandler({
    callAI: (prompt, tier, hint) => callAI(prompt, { tier: tier as ModelTier | undefined, hint }),
    comms,
    execDef,
  ...
  bot.use(async (ctx, next) => {
    if (ctx.chat && (ctx.chat.type === "group" || ctx.chat.type === "supergroup")) {
      console.log(`[exec-node:${role}] Group update: type=${ (ctx as any).updateType} chat=${ctx.chat.id} from=${ctx.from?.username || ctx.from?.id}`);
    }
    await next();
  });
  ...
  const messagePollInterval = setInterval(async () => {
    ...
    try {
      const messages = await comms.pollMessages(new Date(lastPollTime));
      lastPollTime = Date.now();
      for (const msg of messages) {
        const formatted = `<b>[${msg.from_role}]</b>\n${markdownToTelegramHTML(msg.content)}`;
        const chunks = chunkMessage(formatted, 4000);
        for (const chunk of chunks) {
          try {
            await bot.api.sendMessage(execChatId, chunk, { parse_mode: "HTML" });
          } catch {
            await bot.api.sendMessage(execChatId, `[${msg.from_role}]\n${msg.content}`);
          }
        }
      }
    } catch (err) {
