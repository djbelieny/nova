...
export async function processMemoryIntents(
  ...
  // [DEVTASK: projectName | description] — queue a background dev task on a registered codebase
  const devTaskPattern = /\[DEVTASK:\s*([^|\]]+)\s*\|\s*([^\]]+)\]/g;
  let devTaskMatch;
  while ((devTaskMatch = devTaskPattern.exec(clean)) !== null) {
    const projectName = devTaskMatch[1].trim();
    const description = devTaskMatch[2].trim();
    const project = db.getProjectByName(userId, projectName);
    if (project) {
      db.createDevTask(userId, project.id, description);
    } else {
      console.warn(`[memory] DEVTASK: project "${projectName}" not found for user ${userId}`);
    }
    clean = clean.replace(devTaskMatch[0], "");
  }
  ...
  // [SCHEDULE_PAUSE: search text] — pause an active task without deleting it
  for (const match of response.matchAll(/\[SCHEDULE_PAUSE:\s*(.+?)\]/gi)) {
    const task = db.findScheduledTaskByTitle(userId, match[1], ["active"]);
    if (task) {
      db.updateScheduledTask(task.id, { status: "paused" }, userId);
    }
    clean = clean.replace(match[0], "");
  }

  // [SCHEDULE_RESUME: search text] — reactivate a paused task
  for (const match of response.matchAll(/\[SCHEDULE_RESUME:\s*(.+?)\]/gi)) {
    const task = db.findScheduledTaskByTitle(userId, match[1], ["paused"]);
    if (task) {
      const updates: Record<string, any> = { status: "active" };
      // If a recurring task's next trigger has lapsed while paused, roll it forward
      // so it doesn't immediately fire (and possibly spam) on resume.
      if (task.recurrence) {
        const next = parseScheduleTrigger("+1m", userTimezone || "UTC");
        if (next) updates.trigger_at = next.toISOString();
      }
      db.updateScheduledTask(task.id, updates, userId);
    }
    clean = clean.replace(match[0], "");
  }

  // [SCHEDULE_EDIT: search text | TIME: datetime] [| INSTRUCTIONS: text] [| RECUR: rule]
  // Any combination of the three fields may be supplied; at least one is required.
  for (const match of response.matchAll(/\[SCHEDULE_EDIT:\s*(.+?)\]/gi)) {
    const body = match[1];
    const [searchRaw] = body.split("|");
    const searchText = (searchRaw || "").trim();
    const task = searchText
      ? db.findScheduledTaskByTitle(userId, searchText, ["active", "paused"])
      : null;
    ...
    if (task) {
      ...
      const timeMatch = body.match(/\|\s*TIME:\s*([^|]+)/i);
      if (timeMatch) {
        const triggerAt = parseScheduleTrigger(timeMatch[1].trim(), tz);
        if (triggerAt) {
          updates.trigger_at = triggerAt.toISOString();
          updates.next_run_at = triggerAt.toISOString();
        }
      }

      const instrMatch = body.match(/\|\s*INSTRUCTIONS:\s*([^|]+)/i);
      if (instrMatch) updates.instructions = instrMatch[1].trim();

      const recurMatch = body.match(/\|\s*RECUR:\s*([^|]+)/i);
      if (recurMatch) updates.recurrence = recurMatch[1].trim();
...
export async function getMemoryContext(
  ...
  try {
    const card = await getBusinessContextCard(db, userId);
    ...
    if (!allResults.length) return card ? card : "";
    ...
    // Registered codebases — inform Claude so it can use [DEVTASK:] tag
    try {
      const projects = db?.getProjects(userId) ?? [];
      if (projects.length > 0) {
        parts.push(`Registered codebases: ${projects.map((p: any) => p.name).join(", ")}`);
        parts.push(`To queue background dev work: [DEVTASK: projectName | description]`);
      }
    } catch { /* ignore if projects table unavailable */ }
...
/**
 * Returns the nightly-generated Business Context Card (750-token max).
 * Generated by dream mode and cached in service_state.
 * Falls back to empty string if not yet generated.
 */
export async function getBusinessContextCard(
  db: Database | null,
  userId: string
): Promise<string> {
  if (!db) return "";
  const raw = db.getServiceState("business_context_card", userId);
  if (!raw) return "";
  return raw.slice(0, 3000);
}

/**
 * Pre-load relevant context for a predicted query (called by predictor.ts).
 * Returns the relevant context string for the query.
 */
export async function preloadContext(
  ...
  if (!db) return "";
  return getRelevantContext(db, query, userId);
}
