...
export class WhatsAppAdapter implements ChannelAdapter {
  ...
  async start(): Promise<void> {
    ...
    const webhookUrl = process.env.WHATSAPP_WEBHOOK_URL || process.env.MINIAPP_PUBLIC_URL;
    if (webhookUrl) {
      try {
      ...
      console.warn(`[whatsapp:${this.userId}] WHATSAPP_WEBHOOK_URL not set — incoming messages won't work`);
    }
  ...
  async send(chatId: string, message: string | OutgoingMessage): Promise<void> {
    ...
    if (normalizedMsg.voice) {
      const mediaId = await this.uploadMedia(normalizedMsg.voice, "audio/ogg; codecs=opus", "voice.ogg");
      if (mediaId) {
    ...
    if (text && normalizedMsg.buttons?.length && normalizedMsg.buttons.length <= 3) {
      ...
      await this.kapsoPost(`${this.phoneNumberId}/messages`, {
        ...
        interactive: {
          ...
          body: { text },
          action: {
            buttons: normalizedMsg.buttons.map((btn, i) => ({
              type: "reply",
              reply: {
      ...
      let finalText = text;
      if (normalizedMsg.buttons?.length && normalizedMsg.buttons.length > 3) {
        finalText += "\n\nReply with a number:";
        normalizedMsg.buttons.forEach((btn, i) => {
          finalText += `\n${i + 1}. ${btn.label}`;
        });
      }
      await this.kapsoPost(`${this.phoneNumberId}/messages`, {
  ...
  private async uploadMedia(buffer: Buffer | Uint8Array, mimeType: string, filename: string): Promise<string | null> {
    ...
    const bytes = buffer instanceof Buffer ? Array.from(buffer) : Array.from(buffer as Uint8Array);
