...
export class TelegramAdapter implements ChannelAdapter {
  ...
  /** Expose bot instance for backward-compatible operations (orchestrator, miniapp, etc.) */
  getBot(): Bot {
    return this.bot;
  }
  ...
  async send(chatId: string, message: OutgoingMessage): Promise<void> {
    ...
    // Send voice
    if (message.voice) {
      await this.bot.api.sendVoice(numericChatId, new InputFile(message.voice, "response.ogg"));
    }
    ...
    if (message.buttons?.length) {
      ...
      for (let i = 0; i < message.buttons.length; i++) {
        const btn = message.buttons[i];
        keyboard.text(btn.label, btn.callbackData);
        if ((i + 1) % 3 === 0 && i < message.buttons.length - 1) {
          keyboard.row();
        }
      }
    }
    ...
    if (text) {
      const parseMode = message.html ? "HTML" as const : undefined;
      ...
      if (text.length <= MAX_LENGTH) {
        await this.bot.api.sendMessage(numericChatId, text, {
          ...
          await this.bot.api.sendMessage(numericChatId, message.text || text, {
            reply_markup: keyboard,
          });
        });
      } else {
