...
import { resolveCredential } from "./shared-credentials.ts";
...
function getRedirectUri(provider: string, baseUrl: string): string {
  ...
  const base = baseUrl || process.env.DASHBOARD_PUBLIC_URL || "";
  return `${base}/api/integrations/callback`;
}
      ...
      case "google-work": {
        ...
        const tokenRes = await fetch("https://oauth2.googleapis.com/token", {
          ...
          body: new URLSearchParams({
            ...
            redirect_uri: getRedirectUri(provider, process.env.DASHBOARD_PUBLIC_URL || ""),
...
export async function regenerateMcpConfig(
  ...
  if (integrations) {
    for (const integration of integrations) {
      switch (integration.provider) {
        ...
        case "notion": {
          const resolved = resolveCredential(db, userId, "notion");
        ...
        case "gohighlevel": {
          const resolved = resolveCredential(db, userId, "gohighlevel");
          const creds = resolved?.credentials;
          if (!creds) break;
        ...
        case "clickup": {
          const resolved = resolveCredential(db, userId, "clickup");
          const creds = resolved?.credentials;
          if (!creds) break;
          if (creds.api_token) {
  ...
  // Second pass: inherit shared credentials for convertible (non-OAuth-refresh)
  // providers the user has NOT connected themselves. resolveCredential returns
  // the admin's shared credential when the user has no personal integration row.
  // Skip any provider already populated above (user's own takes precedence).
  for (const provider of ["notion", "gohighlevel", "clickup"]) {
    if (mcpServers[provider]) continue;
    const resolved = resolveCredential(db, userId, provider);
    const creds = resolved?.credentials;
    if (!creds) continue;
    ...
    if (provider === "notion") {
      ...
      if (!creds.bearer_token) continue;
      const ghlCmd = mcpCommand("@drausal/gohighlevel-mcp");
      mcpServers["gohighlevel"] = {
        type: "stdio",
        command: ghlCmd.command,
