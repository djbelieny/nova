...
--   telegram_id  → your Telegram user ID (from @userinfobot)
--   name         → your first name
--   timezone     → IANA timezone, e.g. America/New_York
--   phone        → E.164 phone number for Twilio calls/SMS (or NULL)
--   pin          → a private PIN for voice/dashboard auth (or NULL)
--   profile_text → freeform context about you, loaded into every prompt
...
-- Assign all existing rows to the admin user seeded above
UPDATE messages SET user_id = (SELECT id FROM users WHERE telegram_id = 'YOUR_TELEGRAM_ID') WHERE user_id IS NULL;
UPDATE memory SET user_id = (SELECT id FROM users WHERE telegram_id = 'YOUR_TELEGRAM_ID') WHERE user_id IS NULL;
UPDATE logs SET user_id = (SELECT id FROM users WHERE telegram_id = 'YOUR_TELEGRAM_ID') WHERE user_id IS NULL;
UPDATE agent_tasks SET user_id = (SELECT id FROM users WHERE telegram_id = 'YOUR_TELEGRAM_ID') WHERE user_id IS NULL;
UPDATE cost_tracking SET user_id = (SELECT id FROM users WHERE telegram_id = 'YOUR_TELEGRAM_ID') WHERE user_id IS NULL;
