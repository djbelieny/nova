...
const SERVICES: Record<string, ServiceDef> = {
  ...
  voice: {
    ...
    description: "Voice server (port in .env)",
  },
  dashboard: {
    name: "nova-dashboard",
  ...
  dispatcher: {
    name: "nova-task-dispatcher",
    script: "services/task-dispatcher.ts",
    cron: "* * * * *",
    description: "Scheduled task dispatcher (every minute)",
  },
  "health-monitor": {
    name: "nova-health-monitor",
    script: "services/health-monitor.ts",
    cron: "*/30 * * * *",
    description: "Health monitor (every 30 min)",
  },
  "memory-review": {
    name: "nova-memory-review",
  ...
  "exec-research": {
    name: "nova-exec-research",
    script: "src/executive-node.ts",
    description: "Executive Research node",
  },
  "exec-critic": {
    name: "nova-exec-critic",
    script: "src/executive-node.ts",
...
async function installService(config: ServiceDef): Promise<boolean> {
  ...
  // Exec nodes need role-specific env file and --role arg
  const isExec = config.name.startsWith("nova-exec-");
  const role = isExec ? config.name.replace("nova-exec-", "") : null;
  const scriptArgs = role ? ["--", "--role", role] : [];
