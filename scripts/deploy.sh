...
# Pre-deploy guard: parse the dashboard's inline browser <script> blocks.
# A TypeScript-ism inside an inline script compiles server-side but is a browser
# SyntaxError that kills the whole page — catch it here (aborts deploy via set -e).
echo "Validating dashboard inline browser scripts..."
sudo -u nova bash -c "cd /opt/nova && bun run scripts/check-inline-js.ts"
...
echo "Services restarted. Check status with: systemctl status nova-relay nova-voice nova-dashboard nova-exec-*"
