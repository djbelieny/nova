...
> Domains: `your-server.com` (voice/SMS), `your-server.com` (web/dashboard)
...
scp .env.ceo .env.cfo .env.cmo .env.cto .env.coo .env.research .env.critic root@your-server.com:/opt/nova/

# 2. Fix ownership
ssh root@your-server.com "chown nova:nova /opt/nova/.env.*"

# 3. Install and enable systemd services
ssh root@your-server.com "bash /opt/nova/scripts/setup-execs.sh"

# 4. Start all executive services
ssh root@your-server.com "systemctl start nova-exec-ceo nova-exec-cfo nova-exec-cmo nova-exec-cto nova-exec-coo nova-exec-research nova-exec-critic"
```
...
# Service statuses
systemctl status nova-relay nova-voice nova-dashboard
```
...
bun install
systemctl restart nova-relay nova-voice nova-dashboard
```
...
14. Enable services: `systemctl enable --now nova-relay nova-voice nova-dashboard`
